...
function RuntimeErr(originalError, str, lineNo, path6) {
  const lines = str.split("\n");
  const start = Math.max(lineNo - 3, 0);
  const end = Math.min(lines.length, lineNo + 3);
  const filename = path6;
...
function readFile(path6) {
  let res = "";
  try {
    res = fs.readFileSync(path6, "utf8");
  } catch (err) {
    if ((err == null ? void 0 : err.code) === "ENOENT") {
      throw new EtaError(`Could not find template: ${path6}`);
    } else {
      throw err;
    }
  }
...
function getScriptPath() {
  ...
  return path3.dirname(os.platform() === "win32" ? pathUrlPath.slice(1) : pathUrlPath);
}
...
function resolvePath2(filePath, baseDir) {
  ...
  if (filePath.startsWith("~/")) {
    const userHome = os.platform() === "win32" ? process.env.USERPROFILE : process.env.HOME;
    if (!userHome) {
      throw new ReferenceError("No user home directory found");
    }
    return path3.resolve(filePath.replace("~", userHome));
  } else {
...
function createEngine(templateEntryPoint, options = DEFAULT_ENGINE_CONFIG) {
  const resolvedEntryPoint = resolvePath2(templateEntryPoint);
  if (!fs3.existsSync(resolvedEntryPoint)) {
    throw new Error(`[createEngine] Template entry point not found: ${resolvedEntryPoint}`);
  }
  const templatePath = path4.parse(resolvePath2(templateEntryPoint));
  const templateDir = path4.join(templatePath.dir, templatePath.name);
  ...
  for (const filePath of templateFilePaths) {
    ...
    const fullPath = path4.join(templateDir, filePath);
    const fileContent = fs3.readFileSync(fullPath, "utf-8");
    const compiledTemplate = etaInstance.compile(fileContent);
    if (isComponent) {
      const componentName = "@" + filePath.replace(options.extension, "").split(path4.sep).slice(1).join("_");
  ...
  const entryPointCompiledTemplate = etaInstance.compile(fs3.readFileSync(resolvedEntryPoint, {
    encoding: "utf-8",
    flag: "r"
  }));
  return {
    render: (data = {}) => {
      return etaInstance.render(entryPointCompiledTemplate, data);
    },
    loadedTemplates
  };
}
...
function getBuiltinTemplatePath() {
  return resolvePath2(path5.join(getScriptPath(), "templates", "default.eta"));
}
function getInputData(inputPath) {
  const resolvedInputPath = resolvePath2(inputPath);
  if (!fs4.existsSync(resolvedInputPath)) {
    throw new Error(`Input file not found: ${resolvedInputPath}`);
  }
  const rawContent = fs4.readFileSync(resolvedInputPath, "utf-8");
  return import_yaml.default.parse(rawContent);
}
function getOutputPath(inputPath, outputPath) {
  if (outputPath) {
    if (outputPath.indexOf(path5.sep) !== -1) {
      const resolvedOutputPath = resolvePath2(outputPath);
      const parsed = path5.parse(resolvedOutputPath);
      if (parsed.ext === "") {
        return path5.join(parsed.dir, parsed.name + ".html");
      }
      return resolvedOutputPath;
    }
    const parsedInput = path5.parse(inputPath);
    if (outputPath.indexOf(".") !== -1) {
      return path5.join(parsedInput.dir, outputPath);
    }
    return path5.join(parsedInput.dir, outputPath + ".html");
  }
  const parsedPath = path5.parse(inputPath);
  return path5.join(parsedPath.dir, parsedPath.name + ".html");
}
async function main(args) {
  ...
  const inputPath = resolvePath2(parsedArgs._[0]);
